...
/**
 * KST 기준 한국식 12시간제 시간 포맷 (예: "오후 04:23")
 *
 * 📌 왜 필요한가요?
 * toLocaleTimeString('ko-KR')은 브라우저마다 결과가 달라질 수 있어요.
 * 이 함수는 어디서 실행하든 항상 KST 기준으로 동일한 결과를 돌려줍니다.
 *
 * @param date - Date 객체 또는 밀리초 타임스탬프
 * @returns "오후 04:23" 형식의 문자열
 */
export function formatKSTTime12h(date: Date | number): string {
  const d = typeof date === 'number' ? new Date(date) : date
  // UTC 기준으로 9시간 더해서 KST로 변환
  const kst = new Date(d.getTime() + 9 * 60 * 60 * 1000)
  const hours = kst.getUTCHours()
  const minutes = String(kst.getUTCMinutes()).padStart(2, '0')
  // 12시간제: 0~11시는 오전, 12~23시는 오후
  const period = hours < 12 ? '오전' : '오후'
  // 0시 → 12시, 13시 → 1시 등으로 변환
  const h12 = String(hours % 12 || 12).padStart(2, '0')
  return `${period} ${h12}:${minutes}`
}

/**
 * KST 기준 날짜를 "YYYY. M. D." 형식으로 포맷
 *
 * @param date - Date 객체 또는 밀리초 타임스탬프
 * @returns "2025. 1. 20." 형식의 문자열
 */
export function formatKSTDateLocale(date: Date | number): string {
  const d = typeof date === 'number' ? new Date(date) : date
  const kst = new Date(d.getTime() + 9 * 60 * 60 * 1000)
  const year = kst.getUTCFullYear()
  const month = kst.getUTCMonth() + 1
