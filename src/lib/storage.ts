...
export const STORAGE_KEYS = {
  ...
  // 테이블 설정 (컬럼 너비/순서, 사용자별)
  TABLE_SETTINGS_PREFIX: `${PROJECT_PREFIX}_table_settings`,
} as const
...
// ========================================
// 📌 sessionStorage 키 정의 (탭 단위 임시 저장)
// ========================================

/**
 * sessionStorage는 "같은 탭에서만 유효한 임시 메모장"
 * 📌 비유: 카페에서 주문서를 적다가 잠시 자리를 비워도 주문서가 남아있지만,
 *         카페를 나가면(탭 닫기) 주문서가 사라지는 것
 */
export const SESSION_KEYS = {
  // 견적 요청 모달 폼 상태 (닫았다 열어도 유지)
  QUOTE_FORM: `${PROJECT_PREFIX}_quote_form`,
} as const
...
// ========================================
// 📌 sessionStorage 기본 함수 (탭 단위 임시 저장)
// ========================================

/**
 * sessionStorage에 값 저장
 * 📌 SSR 안전: 서버에서는 sessionStorage가 없으므로 window 체크 필수
 */
export function setSessionItem<T>(key: string, value: T): void {
  try {
    ...
    console.error(`[Session] 저장 실패: ${key}`, error)
  }
}
...
export function getSessionItem<T>(key: string, defaultValue: T): T {
  try {
    ...
    console.error(`[Session] 불러오기 실패: ${key}`, error)
    return defaultValue
  }
}

/**
 * sessionStorage에서 값 삭제
 */
export function removeSessionItem(key: string): void {
  try {
    ...
    console.error(`[Session] 삭제 실패: ${key}`, error)
  }
}
